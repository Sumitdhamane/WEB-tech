function getData() {
  ...
  xmlObj.open("GET", "http://localhost:3000/products");
  //step4: send request
  xmlObj.send();
}
...
function deleteProduct() {
  ...
  if (window.confirm(`Are you Sure to delete product with id :${id}`)) {
    ...
    xmlObj.open("DELETE", `http://localhost:3000/products/${id}`);
    xmlObj.send();
  }
}
...
function addProduct() {
  ...
  xmlObj.open("POST", "http://localhost:3000/products");
...
function getSingleProduct() {
  ...
  // step3: create requests
  xmlObj.open("GET", `http://localhost:3000/products/${id}`);
  //step4: send request
  xmlObj.send();
}
...
function updateProduct() {
  ...
  xmlObj.open("PUT", `http://localhost:3000/products/${id}`);
  xmlObj.send(prodJson);
}
